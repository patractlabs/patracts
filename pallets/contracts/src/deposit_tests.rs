...
use frame_support::{
	...
	weights::{
		constants::WEIGHT_PER_SECOND, DispatchClass, DispatchInfo, PostDispatchInfo, Weight,
	},
};
...
use sp_runtime::{
	testing::{Header, H256},
	traits::{BlakeTwo256, Convert, Hash, IdentityLookup, SignedExtension},
...
pub mod test_utils {
	...
	use crate::{
		...
		storage::Storage,
		CodeHash, ContractInfoOf, Pallet as Contracts,
	};
...
parameter_types! {
	...
	pub const DepositPerStorageByte: u64 = 10;
	...
	pub const TestModuleId: ModuleId = ModuleId(*b"py/tests");
}
...
fn deploy_and_call_other_contract() {
	let (callee_wasm, _callee_code_hash) = compile_module::<Test>("return_with_data").unwrap();
...
fn cannot_self_destruct_through_draning() {
	...
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let pre = ChargeDepositPayment::<Test>(PhantomData)
				.pre_dispatch(&ALICE, CALL, &DispatchInfo::default(), 0_usize)
				.unwrap();
			...
			assert!(ChargeDepositPayment::<Test>::post_dispatch(
				pre,
				&DispatchInfo::default(),
				&PostDispatchInfo {
					actual_weight: Some(<<Test as Config>::WeightInfo as WeightInfo>::call(0)),
					pays_fee: Default::default(),
				},
				0_usize,
				&Ok(())
...
fn self_destruct_works() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Balances::free_balance(DJANGO), 1_107_950);
		});
}
...
fn call_return_code() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let deposit_payer = ALICE;

			// Contract has enough total balance in order to not go below the subsistence
			// threshold when transfering 100 balance but this balance is reserved so
			// the transfer still fails but with another return code.
			Balances::make_free_balance_be(&deposit_payer, subsistence + 100);
			Balances::reserve(&deposit_payer, subsistence + 100).unwrap();
			let result = Contracts::bare_call(
				ALICE,
				addr_bob.clone(),
				0,
			...
			Balances::make_free_balance_be(&deposit_payer, subsistence + 1000);
			let result = Contracts::bare_call(
				ALICE,
				addr_bob.clone(),
				0,
...
fn instantiate_return_code() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// Contract has only the minimal balance so any transfer will return BelowSubsistence.
			Balances::make_free_balance_be(&deposit_payer, subsistence);
			let result =
			...
			// the transfer still fails but with another return code.
			Balances::make_free_balance_be(&deposit_payer, subsistence + 10_000);
			Balances::reserve(&deposit_payer, subsistence + 10_000).unwrap();
			let result =
				Contracts::bare_call(ALICE, addr.clone(), 0, GAS_LIMIT, callee_hash.clone())
					.exec_result
					.unwrap();
